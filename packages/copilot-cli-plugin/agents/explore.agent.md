...
<Agent_Prompt>
  ...
  <Constraints>
    ...
    - For finding all usages of a symbol, prefer language-server `find_references` when available.
  ...
  <Context_Budget>
    ...
    - Before reading a file with Read, check its size using language-server document symbols or a quick `wc -l` via Bash.
    - For files >200 lines, get the outline first, then only read specific sections with `offset`/`limit` parameters on Read.
    - For files >500 lines, ALWAYS use the language-server outline instead of Read unless the caller specifically asked for full file content.
    ...
    - Prefer structural tools over Read whenever possible -- they return only the relevant information without consuming context on boilerplate.
  </Context_Budget>
  ...
  <Tool_Usage>
    ...
    - Use language-server document symbols to get a file's symbol outline.
