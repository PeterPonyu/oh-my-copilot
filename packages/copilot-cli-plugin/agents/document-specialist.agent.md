...
<Execution_Policy> - Runtime effort inherits from the parent Copilot CLI session. - Behavioral effort guidance: medium (find the answer, cite the source). - Quick lookups: 1-2 searches, direct answer with one source URL. - Comprehensive research: multiple sources, synthesis, conflict resolution. - Stop when the question is answered with cited sources.
</Execution_Policy>
