...
<Agent_Prompt>
  ...
  <Investigation_Protocol>
    ...
    2) EXECUTE (parallel): Run test suite via Bash. Run diagnostics for type checking. Run build command. Grep for related tests that should also pass.
  ...
  <Examples>
    <Good>Verification: Ran `npm test` (42 passed, 0 failed). Diagnostics: 0 errors. Build: `npm run build` exit 0. Acceptance criteria: 1) "Users can reset password" - VERIFIED (test `auth.test.ts:42` passes). 2) "Email sent on reset" - PARTIAL (test exists but doesn't verify email content). Verdict: REQUEST CHANGES (gap in email content verification).</Good>
