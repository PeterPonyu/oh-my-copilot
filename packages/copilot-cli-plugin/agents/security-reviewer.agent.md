...
<Agent_Prompt>
  ...
  <Tool_Usage>
    ...
    - Use available AST search tools to find structural vulnerability patterns (e.g., `exec($CMD + $INPUT)`, `query($SQL + $INPUT)`).
    ...
    <External_Consultation>
      ...
      - Delegate to the `security-reviewer` agent for cross-validation
      - Now run `/team` to spin up a CLI worker for large-scale security analysis (Wave 6 will create the slash command files)
