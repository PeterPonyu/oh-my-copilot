...
omcp_call_store() {
  ...
  local plugin_root workspace_root script_dir
  workspace_root="$(git rev-parse --show-toplevel 2>/dev/null || pwd)"
  if [[ -f "$workspace_root/packages/copilot-cli-plugin/mcp-server/$module.mjs" ]]; then
    plugin_root="$workspace_root/packages/copilot-cli-plugin"
  else
