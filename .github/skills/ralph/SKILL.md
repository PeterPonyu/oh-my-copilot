---
name: ralph
description: "[OMCP] Self-referential loop until task completion with configurable verification reviewer"
orchestrates-agents: "executor, architect, critic, code-simplifier, debugger"
...
This skill drives a PRD persistence loop over custom-agent roles. It primarily
uses `executor` for implementation, `architect` or `critic` for completion
review, `code-simplifier` for cleanup, and `debugger` for failed verification
loops.
...
7.5 **Mandatory Deslop Pass** (runs unconditionally after Step 7 approval, unless `{{PROMPT}}` contains `--no-deslop`):
   ...
   - **ai-slop-cleaner is a SKILL, not an agent.** Do NOT call it via `Task(subagent_type="oh-my-copilot:ai-slop-cleaner") <!-- TODO: agent ai-slop-cleaner not in v1 list; appears in example/prose only -->` — that subagent type does not exist and the call will fail with "Agent type not found". If you see that error, retry with the Skill tool — do NOT substitute a similarly-named agent like `code-simplifier` as a "closest match".
