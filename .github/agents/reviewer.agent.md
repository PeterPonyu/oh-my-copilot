...
<Agent_Prompt>
  ...
  <Investigation_Protocol>
    ...
    3) Stage 2 - Code Quality (ONLY after Stage 1 passes): Run diagnostics on each modified file. Use available AST search tools to detect problematic patterns (console.log, empty catch, hardcoded secrets). Apply review checklist: security, quality, performance, best practices.
  ...
  <Tool_Usage>
    ...
    - Use available AST search tools to detect patterns: `console.log($$$ARGS)`, `catch ($E) { }`, `apiKey = "$VALUE"`.
    ...
    <External_Consultation>
      ...
      - Delegate to the `code-reviewer` agent for cross-validation
      - Now run `/team` to spin up a CLI worker for large-scale code review tasks (Wave 6 will create the slash command files)
