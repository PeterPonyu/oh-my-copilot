...
usage() {
  cat <<'USAGE'
Usage: scripts/validate-doc-links.sh [--root PATH] [--check-external] [--self-test]
...
public_text_paths=(README.md docs research examples)
...
validate_blueprint_and_examples() {
  ...
  require_contains "examples are labeled illustrative" 'illustrative' examples/copilot-cli-layout/AGENTS.md examples/copilot-cli-layout/.github/copilot-instructions.md examples/copilot-cli-layout/.github/skills/ecosystem-compare/SKILL.md examples/copilot-cli-layout/.github/skills/blueprint-check/SKILL.md
}
...
validate_scope_creep_terms() {
  ...
  if (cd "$ROOT" && grep -RInE --include='*.md' 'cloud|IDE|SDK|runtime|full feature|implemented|parity' README.md docs research examples > "$tmp" || true); then
    if [[ -s "$tmp" ]]; then
      local bad
      bad="$(grep -Eiv 'non-goal|out of scope|future|citation|reference|illustrative|docs-first|research-first|not .*runtime|not .*parity|reject|bound|avoid|no full|no runtime|without claiming|does not claim' "$tmp" || true)"
      if [[ -n "$bad" ]]; then
        printf '%s\n' "$bad" >&2
        rm -f "$tmp"
