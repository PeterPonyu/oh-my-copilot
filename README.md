...
A Copilot CLI-native power pack for GitHub Copilot CLI: 22 agents, 45 skills, 44 typed slash commands, 4 hook events, and a 39-tool MCP server — installed as a single plugin.

## What you get

- **Typed slash commands** — `/omcp:autopilot`, `/omcp:ralph`, `/omcp:ulw-loop`, `/omcp:wiki`, `/omcp:build-fix`, `/omcp:git-master`, `/omcp:cancel`, ... 44 user-invocable routes that the LLM can resolve at the `❯` prompt.
...
| Skills | 45 | autopilot, ralph, ralplan, team, deep-interview, ulw-loop, wiki, build-fix, git-master, cancel, ... |
| Slash commands | 44 | every user-invocable skill has a typed `/omcp:<cmd>` route |
