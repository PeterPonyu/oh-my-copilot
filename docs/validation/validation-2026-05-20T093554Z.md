...
**Generated:** 2026-05-20T09:35:54Z
**Plugin location:** `$HOME/Desktop/oh-my-copilot/packages/copilot-cli-plugin` (source-tree)
...
```
grep '"version"' $HOME/Desktop/oh-my-copilot/packages/copilot-cli-plugin/plugin.json | sed -E 's/.*"version": *"([^"]+)".*/\1/'
...
```
echo '{"jsonrpc":"2.0","id":1,"method":"tools/list"}' | node $HOME/Desktop/oh-my-copilot/packages/copilot-cli-plugin/mcp-server/server.mjs 2>&1 | head -1 | node -e 'let d=""; process.stdin.on("data",c=>d+=c); process.stdin.on("end",()=>{const j=JSON.parse(d.trim()); console.log(j.result.tools.length)})'
...
```
grep -l 'git rev-parse --show-toplevel' $HOME/Desktop/oh-my-copilot/packages/copilot-cli-plugin/scripts/post-tool-audit.sh $HOME/Desktop/oh-my-copilot/packages/copilot-cli-plugin/scripts/log-session-start.sh | wc -l
...
```
MARKER="validation-e2e-$(date +%s%N)" && echo "{\"tool\":\"$MARKER\",\"exit_code\":42,\"stderr\":\"e2e validation\"}" | bash $HOME/Desktop/oh-my-copilot/packages/copilot-cli-plugin/scripts/post-tool-audit.sh > /dev/null && grep -c "$MARKER" $HOME/Desktop/oh-my-copilot/.omcp/traces/default.jsonl
...
```
test -f $HOME/Desktop/oh-my-copilot/packages/copilot-cli-plugin/instructions/AGENTS.md && bash $HOME/Desktop/oh-my-copilot/scripts/check-mirror-drift.sh
...
```
=== check-mirror-drift ===
Generating mirror into /tmp/mirror-check-901891 ...
...
```
cd $HOME/Desktop/oh-my-copilot/packages/copilot-cli-plugin/mcp-server && node --test --test-reporter=tap 'tests/*.test.mjs' 2>&1 | grep -E '^# (tests|pass|fail) ' | tr '\n' ' '
...
_Generated by `scripts/run-validation.sh` at 2026-05-20T09:36:06Z._
