...
| Hooks | GitHub Docs describe hook JSON files under `.github/hooks/` and lifecycle triggers `sessionStart`, `preToolUse`, `postToolUse`, `sessionEnd` (4 events as of v1.0.42). See [`docs/hook-surface.md`](../docs/hook-surface.md) for per-event purpose and the OMC events that have no Copilot equivalent. | v1 can include a conservative hook example as policy/logging illustration, not as product enforcement. |
